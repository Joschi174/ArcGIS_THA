...
const resources = require('./resources-ebb74598.js');
...
const Alert = class {
  constructor(hostRef) {
    ...
    this.resizeObserver = observers.createObserver("resize", (entries) => (this.responsiveContainerWidth = entries[0].contentRect.width));
  ...
  render() {
    ...
    return (index.h(index.Host, { "aria-hidden": dom.toAriaBoolean(hidden), "aria-label": label, "calcite-hydrated-hidden": hidden, role: role }, index.h("div", { class: {
      ...
      ref: this.setTransitionEl }, index.h("div", { class: CSS.contentContainer }, effectiveIcon && greaterOrEqualThanSmall ? this.renderIcon(effectiveIcon) : null, index.h("div", { class: CSS.content }, effectiveIcon && lessThanSmall ? this.renderIcon(effectiveIcon) : null, index.h("div", { class: CSS.textContainer }, index.h("slot", { name: SLOTS.title }), index.h("slot", { name: SLOTS.message }), index.h("slot", { name: SLOTS.link })))), lessThanSmall ? this.renderCloseButton() : null, index.h("div", { class: CSS.footer, hidden: !hasEndActions && !hasQueuedAlerts }, this.renderActionsEnd(), hasQueuedAlerts ? this.renderQueueCount() : null), greaterOrEqualThanSmall ? this.renderCloseButton() : null, open && !queued && autoClose ? index.h("div", { class: CSS.dismissProgress }) : null)));
  }
  renderCloseButton() {
  ...
  renderIcon(icon) {
    return (index.h("div", { class: CSS.icon }, index.h("calcite-icon", { flipRtl: this.iconFlipRtl, icon: icon, scale: this.scale === "l" ? "m" : "s" })));
  }
