...
async function d3(t4, o7, s7) {
  ...
  const n6 = i2({ f: "json", token: o7 }, s7), { data: l9 } = await j(t4, n6), u6 = l9.currentVersion >= 10.4 ? p5(t4, o7, s7) : v2(t4, s7), { defaultTravelMode: f4, supportedTravelModes: c15 } = await u6;
  return l9.defaultTravelMode = f4, l9.supportedTravelModes = c15, m5.fromJSON(l9);
}
async function v2(r3, t4) {
  var _a, _b, _c;
  const i9 = i2({ f: "json" }, t4), { data: l9 } = await j(r3.replace(/\/rest\/.*$/i, "/info"), i9);
  if (!l9 || !l9.owningSystemUrl)
    return { supportedTravelModes: [], defaultTravelMode: null };
  const { owningSystemUrl: u6 } = l9, f4 = bt(u6) + "/sharing/rest/portals/self", { data: c15 } = await j(f4, i9), d4 = t("helperServices.routingUtilities.url", c15);
  if (!d4)
    return { supportedTravelModes: [], defaultTravelMode: null };
  const v3 = f2(u6), p11 = /\/solve$/i.test(v3.path) ? "Route" : /\/solveclosestfacility$/i.test(v3.path) ? "ClosestFacility" : "ServiceAreas", m8 = i2({ f: "json", serviceName: p11 }, t4), h2 = bt(d4) + "/GetTravelModes/execute", g = await j(h2, m8), w2 = [];
  let T2 = null;
...
async function p5(t4, o7, n6) {
  try {
    const r3 = i2({ f: "json", token: o7 }, n6), i9 = bt(t4) + "/retrieveTravelModes", { data: { supportedTravelModes: l9, defaultTravelMode: u6 } } = await j(i9, r3);
...
async function c11(u6, f4, c15) {
  ...
  const B = { ...c15, query: { ...h2.query, ...o6(f4), f: "json" } }, E = R4.endsWith("/solve") ? R4 : `${R4}/solve`, { data: T2 } = await j(E, B);
  return l7(T2);
}
function l7(e2) {
...
//# sourceMappingURL=RouteLayer-BFJ4HEVZ.js.map
