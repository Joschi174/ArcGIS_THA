...
function c(t3) {
  ...
  let d3 = 0;
  const { identifier: l3, version: a4 } = r2(t3, c3, d3);
  ...
  if (d3 = s3(t3, d3, h3), d3 = s3(t3, d3, w3), d3 = s3(t3, d3, g), d3 = s3(t3, d3, p), d3 !== t3.byteLength)
  ...
  for (let e2 = 0; e2 < h3.length; e2++) {
    ...
    for (let o3 = 0; o3 < w3[e2]; o3++) {
      t4 += g[m];
      const e3 = p[m];
      f3[3 * m] = Math.min(u3.maxX, u3.minX + 2 * u3.errorX * t4), f3[3 * m + 1] = Math.min(u3.maxY, u3.minY + 2 * u3.errorY * U), f3[3 * m + 2] = Math.min(u3.maxZ, u3.minZ + 2 * u3.errorZ * e3), m++;
    }
  }
...
function d(o3, r3, n3) {
  ...
  let a4 = 0;
  if (0 === l3)
    a4 = i2.getUint32(1, t2), r3 += 5;
  else if (1 === l3)
    a4 = i2.getUint16(1, t2), r3 += 3;
  else {
    if (2 !== l3)
      throw new s2("lepcc-decode-error", "Bad count type");
    a4 = i2.getUint8(1), r3 += 2;
  }
  if (d3)
    throw new s2("lepcc-decode-error", "LUT not implemented");
  const u3 = Math.ceil(a4 * s5 / 8), f3 = new Uint8Array(o3, r3, u3);
  let h3 = 0, w3 = 0, g = 0;
  const p = -1 >>> 32 - s5;
  for (let e2 = 0; e2 < a4; e2++) {
    for (; w3 < s5; )
      h3 |= f3[g] << w3, w3 += 8, g += 1;
    n3[e2] = h3 & p, h3 >>>= s5, w3 -= s5, w3 + s5 > 32 && (h3 |= f3[g - 1] >> 8 - w3);
  }
  return r3 + g;
}
...
function a3(t3, n3, r3) {
  ...
  for (; i2 < r3; ) {
    ...
    if (u3 < 128)
      ...
      const a4 = (31 & u3) << 6 | 63 & t3[n3 + i2 + 1];
      o3 += String.fromCharCode(a4), i2 += 2;
    } else if (u3 >= 224 && u3 < 240) {
      if (i2 + 2 >= r3)
        throw new s2("utf8-decode-error", "UTF-8 Decode failed. Multi byte character was truncated.");
      const a4 = (15 & u3) << 12 | (63 & t3[n3 + i2 + 1]) << 6 | 63 & t3[n3 + i2 + 2];
      o3 += String.fromCharCode(a4), i2 += 3;
    } else {
      if (!(u3 >= 240 && u3 < 248))
        throw new s2("utf8-decode-error", "UTF-8 Decode failed. Invalid multi byte sequence.");
      {
        if (i2 + 3 >= r3)
          throw new s2("utf8-decode-error", "UTF-8 Decode failed. Multi byte character was truncated.");
        const a4 = (7 & u3) << 18 | (63 & t3[n3 + i2 + 1]) << 12 | (63 & t3[n3 + i2 + 2]) << 6 | 63 & t3[n3 + i2 + 3];
        if (a4 >= 65536) {
          const e2 = 55296 + (a4 - 65536 >> 10), t4 = 56320 + (1023 & a4);
          o3 += String.fromCharCode(e2, t4);
        } else
          o3 += String.fromCharCode(a4);
        i2 += 4;
      }
    }
  }
...
function s4(e2, t3) {
  ...
  for (let o3 = 0; o3 < t3.length; o3++) {
    const i2 = t3[o3], u3 = i2.valueType || i2.type, a4 = A[u3];
    n3.fields[i2.property] = a4(e2, r3), r3 += h2[u3].BYTES_PER_ELEMENT;
  }
  return n3.byteCount = r3, n3;
}
function c2(e2, t3, n3) {
  return f2(e2, t3, n3).map((e3) => {
    const t4 = e3 ? Date.parse(e3) : null;
    return t4 && !Number.isNaN(t4) ? t4 : null;
  });
}
function f2(t3, n3, r3) {
  ...
  for (u3 = 0; u3 < t3; u3 += 1) {
    if (i2 = n3[u3], i2 > 0) {
      if (o3.push(a3(r3, s5, i2 - 1)), 0 !== r3[s5 + i2 - 1])
...
function l2(e2, t3) {
  return new (0, h2[t3.valueType])(e2, t3.byteOffset, t3.count * t3.valuesPerElement);
}
function d2(e2, t3) {
...
function b(n3, r3, o3) {
  ...
  for (let s5 = 0; s5 < r3.ordering.length; s5++) {
    const n4 = r3.ordering[s5], o4 = a(r3[n4]);
    if (o4.count = i2.fields.count ?? 0, "String" === o4.valueType) {
      if (o4.byteOffset = a4, o4.byteCount = i2.fields[n4 + "ByteCount"], "UTF-8" !== o4.encoding)
        throw new s2("unsupported-encoding", "Unsupported String encoding.", { encoding: o4.encoding });
      if (o4.timeEncoding && "ECMA_ISO8601" !== o4.timeEncoding)
        throw new s2("unsupported-time-encoding", "Unsupported time encoding.", { timeEncoding: o4.timeEncoding });
    } else {
      if (!O2(o4.valueType))
        throw new s2("unsupported-value-type", "Unsupported binary valueType", { valueType: o4.valueType });
      {
        const e2 = E(o4.valueType);
        a4 += a4 % e2 != 0 ? e2 - a4 % e2 : 0, o4.byteOffset = a4, o4.byteCount = e2 * o4.valuesPerElement * o4.count;
      }
    }
    a4 += o4.byteCount ?? 0, u3.entries[n4] = o4;
  }
...
function w2(e2, t3) {
  ...
  for (const s5 of t3.ordering) {
    if (!t3.vertexAttributes[s5])
      continue;
    const e3 = { ...t3.vertexAttributes[s5], byteOffset: r3, count: u3 }, n4 = C[s5] || "_" + s5;
    o3.vertexAttributes[n4] = e3, r3 += E(e3.valueType) * e3.valuesPerElement * u3;
  }
  ...
  if (t3.faces && a4) {
    ...
    for (const e3 of t3.ordering) {
      if (!t3.faces[e3])
        continue;
      const n4 = { ...t3.faces[e3], byteOffset: r3, count: a4 };
      o3.faces[e3] = n4, r3 += E(n4.valueType) * n4.valuesPerElement * a4;
    }
  }
  ...
  if (t3.featureAttributes && t3.featureAttributeOrder && c3) {
    ...
    for (const e3 of t3.featureAttributeOrder) {
      ...
      r3 += ("UInt64" === n4.valueType ? 8 : E(n4.valueType)) * n4.valuesPerElement * c3;
    }
  }
  return y(r3, e2.byteLength, "geometry"), o3.byteCount = r3 - o3.byteOffset, o3;
}
...
function I(t3, n3, i2) {
  ...
  if (s5) {
    if ("String" === s5.valueType) {
      const e2 = a4.entries.attributeByteCounts, t4 = l2(n3, e2), r3 = d2(n3, s5);
      return s5.timeEncoding ? c2(e2.count, t4, r3) : f2(e2.count, t4, r3);
    }
    return l2(n3, s5);
  }
...
var A = { Float32: (e2, t3) => new DataView(e2, 0).getFloat32(t3, true), Float64: (e2, t3) => new DataView(e2, 0).getFloat64(t3, true), UInt8: (e2, t3) => new DataView(e2, 0).getUint8(t3), Int8: (e2, t3) => new DataView(e2, 0).getInt8(t3), UInt16: (e2, t3) => new DataView(e2, 0).getUint16(t3, true), Int16: (e2, t3) => new DataView(e2, 0).getInt16(t3, true), UInt32: (e2, t3) => new DataView(e2, 0).getUint32(t3, true), Int32: (e2, t3) => new DataView(e2, 0).getInt32(t3, true) };
function O2(e2) {
  return h2.hasOwnProperty(e2);
}
function E(e2) {
  return O2(e2) ? h2[e2].BYTES_PER_ELEMENT : 0;
}
...
//# sourceMappingURL=chunk-VMLEOD66.js.map
