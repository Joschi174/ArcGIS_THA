...
async function r2(r3, n2, t, s3, a2, i2) {
  let l = null;
  if (null != t) {
    const o2 = `${r3}/nodepages/`, n3 = o2 + Math.floor(t.rootIndex / t.nodesPerPage);
    try {
      return { type: "page", rootPage: (await j(n3, { query: { f: "json", token: s3 }, responseType: "json", signal: i2 })).data, rootIndex: t.rootIndex, pageSize: t.nodesPerPage, lodMetric: t.lodSelectionMetricType, urlPrefix: o2 };
    } catch (c) {
      null != a2 && a2.warn("#fetchIndexInfo()", "Failed to load root node page. Falling back to node documents.", n3, c), l = c;
    }
  }
  if (!n2)
    return null;
  const d = `${r3}/nodes/`, p3 = d + (n2 && n2.split("/").pop());
  try {
    return { type: "node", rootNode: (await j(p3, { query: { f: "json", token: s3 }, responseType: "json", signal: i2 })).data, urlPrefix: d };
  } catch (c) {
...
var N = (o2) => {
  let N2 = class extends o2 {
    ...
    _updateExtent(e2) {
      ...
      if ("page" === (e2 == null ? void 0 : e2.type)) {
        ...
        if (o3.obb.center[0] < E || null == this.fullExtent || this.fullExtent.hasZ)
          return;
        const i2 = o3.obb.halfSize, s3 = o3.obb.center[2], a2 = Math.sqrt(i2[0] * i2[0] + i2[1] * i2[1] + i2[2] * i2[2]);
    ...
    async _saveAs(e2, t) {
      ...
      const s3 = i2.portal || j2.getDefault();
      await this._ensureLoadBeforeSave(), i2.type = T, i2.portal = s3;
      const a2 = { origin: "portal-item", url: null, messages: [], portal: s3, portalItem: i2, writtenProperties: [], blockedRelativeUrls: [], resources: { toAdd: [], toUpdate: [], toKeep: [], pendingOperations: [] } }, n2 = { layers: [this.write({}, a2)] };
      return await Promise.all(a2.resources.pendingOperations ?? []), await this._validateAgainstJSONSchema(n2, a2, o3), i2.url = this.url, i2.title || (i2.title = this.title), this._updateTypeKeywords(i2, o3, U.newItem), await s3.signIn(), await ((_a = s3.user) == null ? void 0 : _a.addItem({ item: i2, folder: o3 && o3.folder, data: n2 })), await p2(this.resourceReferences, a2, null), this.portalItem = i2, i(a2), a2.portalItem = i2, i2;
    }
    async _save(e2) {
      const t = { ...O, ...e2 };
      if (!this.portalItem)
      ...
      const o3 = { origin: "portal-item", url: this.portalItem.itemUrl && L(this.portalItem.itemUrl), messages: [], portal: this.portalItem.portal || j2.getDefault(), portalItem: this.portalItem, writtenProperties: [], blockedRelativeUrls: [], resources: { toAdd: [], toUpdate: [], toKeep: [], pendingOperations: [] } }, i2 = { layers: [this.write({}, o3)] };
    ...
    async _validateAgainstJSONSchema(e2, t, o3) {
      ...
      const s3 = o3 == null ? void 0 : o3.validationOptions, a2 = s3 == null ? void 0 : s3.enabled, n2 = u();
      if (a2 && n2) {
        const t2 = (await n2()).validate(e2, o3.portalItemLayerType);
        if (t2.length > 0) {
          const e3 = `Layer item did not validate:
${t2.join("\n")}`;
          if (A.error(`_validateAgainstJSONSchema(): ${e3}`), "throw" === s3.failPolicy) {
...
//# sourceMappingURL=chunk-CQXEW2DK.js.map
