  ...
  _changed() {
    if (super._changed(), this._clear) {
      ...
      if (t > 0 && i > 0) {
        ...
        s = ne(s, 0, Math.min(t / 2, i / 2));
        let o = this.get("pointerX", 0), l = this.get("pointerY", 0), r = this.get("pointerBaseWidth", 15) / 2, a = 0, n = 0, h = 0, p2 = (o - a) * (i - n) - (l - n) * (t - a), u2 = (o - h) * (0 - i) - (l - i) * (t - h);
        const g = this._display;
        if (g.moveTo(s, 0), p2 > 0 && u2 > 0) {
          let i2 = Math.round(ne(o, s + r, t - r - s));
          l = ne(l, -1 / 0, 0), g.lineTo(i2 - r, 0), g.lineTo(o, l), g.lineTo(i2 + r, 0);
        }
        if (g.lineTo(t - s, 0), g.arcTo(t, 0, t, s, s), p2 > 0 && u2 < 0) {
          let a2 = Math.round(ne(l, s + r, i - r - s));
          o = ne(o, t, 1 / 0), g.lineTo(t, s), g.lineTo(t, Math.max(a2 - r, s)), g.lineTo(o, l), g.lineTo(t, a2 + r);
        }
        if (g.lineTo(t, i - s), g.arcTo(t, i, t - s, i, s), p2 < 0 && u2 < 0) {
          let a2 = Math.round(ne(o, s + r, t - r - s));
          l = ne(l, i, 1 / 0), g.lineTo(t - s, i), g.lineTo(a2 + r, i), g.lineTo(o, l), g.lineTo(a2 - r, i);
        }
        if (g.lineTo(s, i), g.arcTo(0, i, 0, i - s, s), p2 < 0 && u2 > 0) {
          let t2 = Math.round(ne(l, s + r, i - s - r));
          o = ne(o, -1 / 0, 0), g.lineTo(0, i - s), g.lineTo(0, t2 + r), g.lineTo(o, l), g.lineTo(0, Math.max(t2 - r, s));
        }
...
Object.defineProperty(p, "className", { enumerable: true, configurable: true, writable: true, value: "PointedRectangle" }), Object.defineProperty(p, "classNames", { enumerable: true, configurable: true, writable: true, value: Re.classNames.concat([p.className]) });
var u = class extends Je {
  ...
  _afterNew() {
    this._settings.themeTags = Kt(this._settings.themeTags, ["tooltip"]), super._afterNew(), this.set("background", p.new(this._root, { themeTags: ["tooltip", "background"] })), this._label = this.children.push(Qe.new(this._root, {})), this._disposers.push(this._label.events.on("boundschanged", () => {
  ...
  _changed() {
    if (super._changed(), (this.isDirty("pointTo") || this.isDirty("pointerOrientation")) && this._updateBackground(), this.isDirty("tooltipTarget") && this.updateBackgroundColor(), this.isDirty("keepTargetHover")) {
      if (this.get("keepTargetHover")) {
        ...
        this._keepHoverDp = new A([t.events.on("pointerover", (t2) => {
  ...
  _updateTextColor(t) {
    this.get("autoTextColor") && (null == t && (t = this.get("background").get("fill")), null == t && (t = this._root.interfaceColors.get("background")), t instanceof Xt && this.label.set("fill", Xt.alternative(t, this._root.interfaceColors.get("alternativeText"), this._root.interfaceColors.get("text"))));
  }
  _setDataItem(t) {
  ...
  _updateBackground() {
    ...
    if (t) {
      ...
      l instanceof K && (s = l.value);
      let r = t.width(), a = t.height(), n = this.parent, u2 = 0, g = 0;
      if (n) {
        u2 = n.x(), g = n.y();
        const t2 = n.get("layerMargin");
        t2 && (u2 += t2.left || 0, g += t2.top || 0, r += (t2.left || 0) + (t2.right || 0), a += (t2.top || 0) + (t2.bottom || 0));
      }
      const d = this.get("bounds", { left: -u2, top: -g, right: r - u2, bottom: a - g });
      this._updateBounds();
      let c = this.width(), b = this.height();
      0 === c && (c = this._w), 0 === b && (b = this._h);
      let _ = this.get("pointTo", { x: r / 2, y: a / 2 }), f = _.x, m = _.y, v2 = this.get("pointerOrientation"), T = this.get("background"), y = 0, D = 0, k = 0;
      T instanceof p && (y = T.get("pointerLength", 0), D = T.get("strokeWidth", 0) / 2, k = D, T.set("width", c), T.set("height", b));
      let w = 0, x = 0, C = d.right - d.left, P = d.bottom - d.top;
      "horizontal" == v2 || "left" == v2 || "right" == v2 ? (D = 0, "horizontal" == v2 ? f > d.left + C / 2 ? (f -= c * (1 - i) + y, k *= -1) : f += c * i + y : "left" == v2 ? f += c * (1 - i) + y : (f -= c * i + y, k *= -1)) : (k = 0, "vertical" == v2 ? m > d.top + b / 2 + y ? m -= b * (1 - s) + y : (m += b * s + y, D *= -1) : "down" == v2 ? m -= b * (1 - s) + y : (m += b * s + y, D *= -1)), f = ne(f, d.left + c * i, d.left + C - c * (1 - i)) + k, m = ne(m, d.top + b * s, d.top + P - b * (1 - s)) - D, w = _.x - f + c * i + k, x = _.y - m + b * s - D, this._fx = f, this._fy = m;
...
Object.defineProperty(u, "className", { enumerable: true, configurable: true, writable: true, value: "Tooltip" }), Object.defineProperty(u, "classNames", { enumerable: true, configurable: true, writable: true, value: Je.classNames.concat([u.className]) });

export {
  u
};
//# sourceMappingURL=chunk-PZBCO4WH.js.map
